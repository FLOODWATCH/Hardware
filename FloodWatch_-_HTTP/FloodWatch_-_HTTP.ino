...
const char* ssid = "LITONJUA";
const char* password = "LUNA12345";
...
void loop() {
  ...
  if (WiFi.status() == WL_CONNECTED) { //if connected to wifi, read the ff codes || conditions
    ...
    String display;
    if(distance > 90 || distance == 0){ //LEVEL 0 //greater than 4 meters
      //LEVEL 0 -> No SMS && NO HTTP Request
      Serial.println("The Water Level Is Normal, Have a great Day!");  

        
    }else if(distance > 67 && distance <= 90 ){ //LEVEL 1 //greater than 3 meters and less than or equal to 4 meters
